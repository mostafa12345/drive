...
def get_new_title(title, parent_name, document=False, folder=False):
    """
    Returns an error if a file or folder with the same title exists at the same level.
    :param title: Title of entity to be checked.
    :param parent_name: Parent entity of the entity to be checked.
    :param document: Boolean indicating if the entity is a document.
    :param folder: Boolean indicating if the entity is a folder.
    :return: Original title if no duplicate exists, else throws an error.
    """

    entity_title, entity_ext = os.path.splitext(title)

    # Define filters for duplicate checking
    filters = {
        "is_active": 1,
        "parent_drive_entity": parent_name,
        "title": title,  # Check for exact match
    }

    # Adjust filters for specific cases
    if document:
        filters["mime_type"] = "frappe_doc"
    elif folder:
        filters["is_group"] = 1
    else:
    ...
    # Check for existing entities with the same title
    duplicate_entity = frappe.db.exists("Drive Entity", filters)

    # If a duplicate exists, throw an error
    if duplicate_entity:
        frappe.msgprint(
            _("A file or folder with the name '{0}' already exists in this location. Please choose a different name.").format(title),
            alert=True
        )
        frappe.throw(_("File or folder already exists."))


    # Return the original title if no duplicate exists
    return title
